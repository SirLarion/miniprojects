...
void App::renderSkeleton() {
	...
	for (auto i = 0u; i < transforms.size(); ++i) {
		...
		Vec3f right_v, up_v, ahead_v;

		right = Vec4f(transforms[i].getCol(0)).getXYZ();
		right_v = joint_world_pos + right.normalized() * scale;

		up = Vec4f(transforms[i].getCol(1)).getXYZ();
		up_v = joint_world_pos + up.normalized() * scale;

		ahead = Vec4f(transforms[i].getCol(2)).getXYZ();
		ahead_v = joint_world_pos + ahead.normalized() * scale;
		...
		glColor3f(1, 0, 0); // red
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z); glVertex3f(right_v.x, right_v.y, right_v.z);

		glColor3f(0, 1, 0); // green
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z); glVertex3f(up_v.x, up_v.y, up_v.z);

		glColor3f(0, 0, 1); // blue
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z); glVertex3f(ahead_v.x, ahead_v.y, ahead_v.z);

		int parent = skel_.getJointParent(i);
		if (parent != -1) {
			Vec3f parent_world_pos = Vec4f(transforms[parent].getCol(3)).getXYZ();;
			glColor3f(1, 1, 1);
			glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z); 
...
vector<Vertex> App::computeSSD(const vector<WeightedVertex>& source_vertices) {
	...
	for (const auto& sv : source_vertices) {
		...
		for (auto i = 0u; i < WEIGHTS_PER_VERTEX; i++) {
			Mat4f TB = ssd_transforms[sv.joints[i]];
			v.position += sv.weights[i] * (TB * sv.position);
			v.normal += sv.weights[i] * (TB * Vec4f(sv.normal, 0)).getXYZ();
		}
		v.color = sv.color;
