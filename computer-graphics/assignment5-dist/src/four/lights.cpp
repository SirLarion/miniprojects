...
void DirectionalLight::getIncidentIllumination(const Vec3f& p,
	...
	dir_to_light = (-direction_).normalized();
...
void PointLight::getIncidentIllumination(const Vec3f& p,
	...
	float r = (position_ - p).length();
	distance = r;

	incident_intensity = 1.0f / 
		(quadratic_attenuation_ * r*r + 
		 linear_attenuation_    * r + 
		 constant_attenuation_) * intensity_;
