...
bool Plane::intersect( const Ray& r, Hit& h, float tmin ) const {
	...
	auto n = normal();
	float n_rd = n.dot(r.direction);
	float n_ro = n.dot(r.origin);

	if(n_rd != 0) {
		float t = (offset() - n_ro) / n_rd;
		if (t >= tmin && t < h.t) {
			h.set(t, material(), n);
			return true;
		}
...
bool Triangle::intersect(const Ray& r, Hit& h, float tmin) const {
	...
	Vec3f n = (C-A).cross(B-A).normalized();
	float n_rd = n.dot(r.direction);
	float n_ro = n.dot(r.origin);

	if(n_rd != 0) {
		float t = (n.dot(A) - n_ro) / n_rd;
		if (t >= tmin && t < h.t) {
			Vec2f p = r.pointAtParameter(t).getXY();
			Vec2f a = A.getXY();
			Mat2f mat = Mat2f();
			mat.setCol(0, B.getXY() - a);
			mat.setCol(1, C.getXY() - a);
			Vec2f w = mat.inverted() * (p - a);
