		...
		bool shade_back) const
{
	...
	Vec3f answer = Vec3f(0);
	Vec3f p = ray.pointAtParameter(hit.t);

	Vec3f n = hit.normal;
	float n_dot = n.dot(dir_to_light);

	Vec3f r = -dir_to_light - 2 * n.dot(-dir_to_light) * n;
	float r_dot = std::max((float)r.dot(-ray.direction), 0.0f);

	// is light behind object
	if (n_dot < 0) {
		if (shade_back) {
			n_dot = -n_dot;
		}
	...
	Vec3f diffuse = n_dot * diffuse_color(p);
	Vec3f specular = FW::pow(r_dot, exponent()) * specular_color();

	answer = incident_intensity * (diffuse + specular);
