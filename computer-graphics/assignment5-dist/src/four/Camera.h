...
public:
	...
	static inline FW::Vec2f normalizedImageCoordinateFromPixelCoordinate(const FW::Vec2f& pixel, const FW::Vec2i& imageSize) {
		...
		return FW::Vec2f((2*pixel.x)/imageSize.x -1, 1 - (2*pixel.y)/imageSize.y);
	}
...
public:
	...
	virtual Ray generateRay(const FW::Vec2f& point) {
		...
		auto x = point.x * size/2 * horizontal;
		auto y = point.y * size/2 * up;
		return Ray(center + x + y, this->direction);
	}
...
public:
	...
	{	
		...
		float d = 1.0 / FW::tan(fov_angle / 2);
		auto x = point.x * horizontal;
		auto y = point.y * up;
		auto r = x + y + d * direction;

		return Ray(center + r, r.normalized());
	}
