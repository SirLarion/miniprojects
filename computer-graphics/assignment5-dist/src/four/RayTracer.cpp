...
Vec3f RayTracer::traceRay(Ray& ray, float tmin, int bounces, float refr_index, Hit& hit, FW::Vec3f debug_color) const {
	...
	for (auto i = 0u; i < scene_.getNumLights(); i++) {
		auto li = scene_.getLight(i);
		Vec3f d2l, intensity;
		float distance;
		li->getIncidentIllumination(point, d2l, intensity, distance);

		answer += m->shade(ray, hit, d2l, intensity, args_.shade_back);
	}
